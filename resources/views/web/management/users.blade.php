...
<section class="panel-section">
    ...
    <form method="POST" action="{{ route('tenant.users.store') }}" class="filters-grid">
        @csrf
        <div>
        ...
        <table>
            ...
            @forelse($rows as $row)
                ...
                <tr>
                    ...
                    <td>
                        @if($canManage)
                            <form method="POST" action="{{ route('tenant.users.assignment', ['managedUser' => $row->id]) }}" class="filters-grid" style="grid-template-columns: repeat(3, minmax(0, 1fr));">
                                @csrf
                                <div>
                                    <label>Peran</label>
                    ...
                    <td>
                        @if($ownerMode && $row->id !== $user->id)
                            <form method="POST" action="{{ route('tenant.users.archive', ['managedUser' => $row->id]) }}" class="inline-form">
                                @csrf
        ...
        <table>
            ...
            @forelse($archivedRows as $row)
                <tr>
                    ...
                    <td>
                        <form method="POST" action="{{ route('tenant.users.restore', ['managedUser' => $row->id]) }}" class="inline-form">
                            @csrf
