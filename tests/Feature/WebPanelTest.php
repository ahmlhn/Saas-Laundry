...
class WebPanelTest extends TestCase
{
    ...
    public function test_admin_can_login_and_open_web_panel_pages(): void
    {
        ...
        $login->assertRedirect(route('filament.tenant.pages.dashboard'));

        $this->followRedirects($login)
            ->assertOk()
            ->assertSeeText('Ringkasan Tenant');

        $this->get('/orders')->assertRedirect($this->filamentOrderIndexUrl());
        $this->get('/orders/create')->assertRedirect($this->filamentOrderCreateUrl());
        $this->get('/orders/'.$this->orderA->id)->assertRedirect($this->filamentOrderViewUrl($this->orderA));
        $this->get('/users')->assertRedirect(UserResource::getUrl(name: 'index', panel: 'tenant'));
        $this->get('/customers')->assertRedirect(CustomerResource::getUrl(name: 'index', panel: 'tenant'));
        $this->get('/services')->assertRedirect(ServiceResource::getUrl(name: 'index', panel: 'tenant'));
        $this->get('/billing')->assertRedirect(BillingPage::getUrl(panel: 'tenant'));
        $this->get('/outlet-services')->assertRedirect(OutletServiceResource::getUrl(name: 'index', panel: 'tenant'));
        $this->get('/outlets')->assertRedirect(OutletResource::getUrl(name: 'index', panel: 'tenant'));
        $this->get('/shipping-zones')->assertRedirect(ShippingZoneResource::getUrl(name: 'index', panel: 'tenant'));
    ...
    public function test_wa_page_is_plan_gated(): void
    {
        ...
        $this->actingAs($this->admin, 'web')
            ->get('/wa')
            ->assertRedirect(WhatsAppPage::getUrl(panel: 'tenant'));
    ...
    {
        $this->actingAs($this->admin, 'web')
            ->get('/orders/'.$this->foreignOrderId)
            ->assertNotFound();
    }
    ...
    {
        $this->actingAs($this->admin, 'web')
            ->get('/orders/'.$this->orderA->id.'/receipt')
            ->assertOk()
            ->assertSeeText('Cetak Ringkas Transaksi')
            ->assertSeeText($this->orderA->order_code)
            ->assertSee($this->filamentOrderViewUrl($this->orderA), false);
    }

    public function test_receipt_page_respects_tenant_scope(): void
    {
        $this->actingAs($this->admin, 'web')
            ->get('/orders/'.$this->foreignOrderId.'/receipt')
            ->assertNotFound();
    }

    public function test_legacy_order_routes_redirect_to_filament_pages(): void
    {
        $this->actingAs($this->admin, 'web')
            ->get('/orders/'.$this->orderA->id)
            ->assertRedirect($this->filamentOrderViewUrl($this->orderA));

        $this->actingAs($this->admin, 'web')
            ->get('/orders/create')
            ->assertRedirect($this->filamentOrderCreateUrl());
    }
    ...
    public function test_admin_can_export_order_board_csv_with_filters_and_scope(): void
    {
        ...
        $response = $this->actingAs($this->admin, 'web')
            ->get('/orders/export?laundry_status=ironing');
    ...
    public function test_owner_export_order_board_csv_includes_all_outlets_in_tenant_scope(): void
    {
        ...
        $response = $this->actingAs($this->owner, 'web')
            ->get('/orders/export?laundry_status=received');
    ...
    public function test_admin_can_export_billing_outlet_csv_with_scope(): void
    {
        ...
        $response = $this->actingAs($this->admin, 'web')
            ->get('/billing/export?period='.$period.'&dataset=outlets');
    ...
    public function test_owner_can_export_billing_usage_csv_by_selected_period(): void
    {
        ...
        $response = $this->actingAs($this->owner, 'web')
            ->get('/billing/export?period='.$period.'&dataset=usage');
    ...
    public function test_owner_can_filter_billing_by_outlet_scope(): void
    {
        ...
        $this->actingAs($this->owner, 'web')
            ->get('/billing?period='.$period.'&outlet_id='.$this->outletB->id)
            ->assertOk()
    ...
    public function test_admin_can_filter_billing_by_payment_status(): void
    {
        ...
        $this->actingAs($this->admin, 'web')
            ->get('/billing?period='.$period.'&payment_status=paid')
            ->assertOk()
        ...
        $this->actingAs($this->admin, 'web')
            ->get('/billing?period='.$period.'&payment_status=unpaid')
            ->assertOk()
            ->assertViewHas('selectedPaymentStatus', fn ($selectedPaymentStatus): bool => $selectedPaymentStatus === 'unpaid')
            ->assertViewHas('ordersCount', fn (int $ordersCount): bool => $ordersCount === 1);

        $this->actingAs($this->admin, 'web')
            ->get('/billing?period='.$period.'&payment_status=partial')
            ->assertOk()
    ...
    public function test_owner_can_see_invoice_aging_report_summary_by_filter_scope(): void
    {
        ...
        $this->actingAs($this->owner, 'web')
            ->get('/billing?period='.$period.'&outlet_id='.$this->outletB->id.'&payment_status=unpaid')
            ->assertOk()
    ...
    public function test_owner_can_export_invoice_aging_csv_with_filters(): void
    {
        ...
        $response = $this->actingAs($this->owner, 'web')
            ->get('/billing/export?period='.$period.'&outlet_id='.$this->outletB->id.'&payment_status=unpaid&dataset=aging');
    ...
    public function test_owner_can_filter_aging_detail_by_bucket(): void
    {
        ...
        $this->actingAs($this->owner, 'web')
            ->get('/billing?period='.$period.'&outlet_id='.$this->outletB->id.'&payment_status=unpaid&aging_bucket=d31_plus')
            ->assertOk()
    ...
    public function test_owner_can_export_aging_detail_csv_with_bucket_filter(): void
    {
        ...
        $response = $this->actingAs($this->owner, 'web')
            ->get('/billing/export?period='.$period.'&outlet_id='.$this->outletB->id.'&payment_status=unpaid&aging_bucket=d8_14&dataset=aging_details');
    ...
    public function test_owner_can_export_billing_order_detail_csv_with_outlet_filter(): void
    {
        ...
        $response = $this->actingAs($this->owner, 'web')
            ->get('/billing/export?period='.$period.'&outlet_id='.$this->outletB->id.'&dataset=orders');
    ...
    public function test_owner_can_export_billing_order_detail_csv_with_payment_status_filter(): void
    {
        ...
        $response = $this->actingAs($this->owner, 'web')
            ->get('/billing/export?period='.$period.'&payment_status=paid&dataset=orders');
    ...
    public function test_admin_can_update_aging_collection_workflow_from_billing_panel(): void
    {
        ...
        $this->actingAs($this->admin, 'web')
            ->post('/billing/collections/'.$order->id, [
    ...
    public function test_owner_can_filter_aging_detail_by_collection_status_and_export_cash_daily_csv(): void
    {
        ...
        $this->actingAs($this->owner, 'web')
            ->get('/billing?period='.$period.'&outlet_id='.$this->outletB->id.'&payment_status=partial&collection_status=escalated')
            ->assertOk()
        ...
        $response = $this->actingAs($this->owner, 'web')
            ->get('/billing/export?period='.$period.'&outlet_id='.$this->outletB->id.'&cash_date='.$cashDate.'&dataset=cash_daily');
    ...
    public function test_admin_can_create_order_from_web_transaction_form(): void
    {
        ...
        $response->assertRedirect($this->filamentOrderViewUrl($order));
    ...
    public function test_admin_can_add_payment_from_web_order_detail(): void
    {
        ...
        $response = $this->actingAs($this->admin, 'web')
            ->post('/orders/'.$this->walkInOrder->id.'/payments', [
                'amount' => 7000,
                'method' => 'cash',
                'notes' => 'DP tahap 2',
            ]);

        $response->assertRedirect($this->filamentOrderViewUrl($this->walkInOrder));
    ...
    public function test_quick_payment_action_rejects_already_paid_order(): void
    {
        ...
        $this->actingAs($this->admin, 'web')
            ->from('/orders/'.$paidOrder->id)
            ->post('/orders/'.$paidOrder->id.'/payments', [
                'method' => 'cash',
                'quick_action' => 'full',
            ])
            ->assertRedirect('/orders/'.$paidOrder->id)
    ...
    public function test_admin_cannot_add_payment_for_order_outside_scope(): void
    {
        ...
        $this->actingAs($this->admin, 'web')
            ->post('/orders/'.$this->foreignOrderId.'/payments', [
                'amount' => 5000,
                'method' => 'transfer',
            ])
            ->assertNotFound();
    ...
    public function test_admin_can_update_laundry_status_from_order_detail(): void
    {
        ...
        $this->actingAs($this->admin, 'web')
            ->post('/orders/'.$this->orderA->id.'/status/laundry', [
                'laundry_status' => 'ready',
            ])
            ->assertRedirect($this->filamentOrderViewUrl($this->orderA));
    ...
    public function test_laundry_status_guard_shows_invalid_transition_reason_on_detail_order(): void
    {
        ...
        $this->actingAs($this->admin, 'web')
            ->from('/orders/'.$this->orderA->id)
            ->post('/orders/'.$this->orderA->id.'/status/laundry', [
                'laundry_status' => 'completed',
            ])
            ->assertRedirect('/orders/'.$this->orderA->id)
    ...
    public function test_admin_can_update_courier_status_from_order_detail(): void
    {
        ...
        $this->actingAs($this->admin, 'web')
            ->post('/orders/'.$this->courierOrderReady->id.'/status/courier', [
                'courier_status' => 'delivery_pending',
            ])
            ->assertRedirect($this->filamentOrderViewUrl($this->courierOrderReady));
    ...
    public function test_courier_status_guard_shows_laundry_not_ready_reason_on_detail_order(): void
    {
        ...
        $this->actingAs($this->admin, 'web')
            ->from('/orders/'.$this->courierOrderBlocked->id)
            ->post('/orders/'.$this->courierOrderBlocked->id.'/status/courier', [
                'courier_status' => 'delivery_pending',
            ])
            ->assertRedirect('/orders/'.$this->courierOrderBlocked->id)
    ...
    public function test_admin_can_assign_courier_from_order_detail(): void
    {
        ...
        $this->actingAs($this->admin, 'web')
            ->post('/orders/'.$this->orderA->id.'/assign-courier', [
    ...
    public function test_admin_cannot_assign_courier_for_order_outside_scope(): void
    {
        ...
        $this->actingAs($this->admin, 'web')
            ->post('/orders/'.$this->foreignOrderId.'/assign-courier', [
                'courier_user_id' => $this->courier->id,
            ])
            ->assertNotFound();
    }
    ...
    public function test_admin_can_apply_bulk_laundry_status_update(): void
    {
        ...
        $this->actingAs($this->admin, 'web')
            ...
            ->assertRedirect($this->filamentOrderIndexUrl());
    ...
    public function test_bulk_update_keeps_bulk_report_payload_for_filament_index(): void
    {
        $this->actingAs($this->admin, 'web')
            ->post('/orders/bulk-update', [
                'action' => 'mark-ready',
    ...
    public function test_owner_can_archive_and_restore_user_from_web_management(): void
    {
        ...
        $this->actingAs($this->owner, 'web')
            ->post('/users/'.$this->admin->id.'/archive')
        ...
        $this->actingAs($this->owner, 'web')
            ->post('/users/'.$this->admin->id.'/restore')
            ->assertRedirect(route('tenant.users.index'));
    ...
    public function test_owner_can_invite_and_reassign_user_from_web_management(): void
    {
        ...
        $this->actingAs($this->owner, 'web')
            ->post('/users/'.$invited->id.'/assignment', [
                'status' => 'inactive',
                'role_key' => 'courier',
    ...
    public function test_admin_cannot_manage_forbidden_roles_or_out_of_scope_outlets(): void
    {
        ...
        $this->actingAs($this->admin, 'web')
            ->post('/users/'.$this->owner->id.'/assignment', [
                'status' => 'active',
                'role_key' => 'cashier',
                'outlet_ids' => [$this->outletA->id],
            ])
        ...
        $this->actingAs($this->admin, 'web')
            ->from('/users')
            ->post('/users/'.$this->courier->id.'/assignment', [
                'status' => 'active',
                'role_key' => 'courier',
                'outlet_ids' => [$this->outletA->id, $this->outletB->id],
            ])
    ...
    public function test_owner_can_archive_and_restore_outlet_from_web_management(): void
    {
        ...
        $this->actingAs($this->owner, 'web')
            ->post('/outlets/'.$this->outletB->id.'/archive')
            ->assertRedirect(route('tenant.outlets.index'));
        ...
        $this->actingAs($this->owner, 'web')
            ->post('/outlets/'.$this->outletB->id.'/restore')
            ->assertRedirect(route('tenant.outlets.index'));
    ...
    public function test_owner_lifecycle_guards_block_self_user_and_last_outlet_archive(): void
    {
        ...
        $this->actingAs($this->owner, 'web')
            ->from('/users')
            ->post('/users/'.$this->owner->id.'/archive')
            ->assertRedirect('/users')
        ...
        $this->actingAs($this->owner, 'web')
            ->from('/outlets')
            ->post('/outlets/'.$this->outletA->id.'/archive')
    ...
    public function test_admin_cannot_execute_owner_only_lifecycle_actions(): void
    {
        ...
        $this->actingAs($this->admin, 'web')
            ->post('/users/'.$this->cashier->id.'/archive')
            ->assertStatus(403);
    ...
    public function test_admin_can_archive_and_restore_customer_and_service_from_web_management(): void
    {
        ...
        $this->actingAs($this->admin, 'web')
            ->post('/customers/'.$this->customer->id.'/restore')
            ->assertRedirect(route('tenant.customers.index'));
        ...
        $this->actingAs($this->admin, 'web')
            ->post('/services/'.$this->service->id.'/archive')
            ->assertRedirect(route('tenant.services.index'));
        ...
        $this->actingAs($this->admin, 'web')
            ->post('/services/'.$this->service->id.'/restore')
            ->assertRedirect(route('tenant.services.index'));
    ...
    public function test_admin_can_create_and_toggle_shipping_zone_from_web_panel(): void
    {
        ...
        $this->actingAs($this->admin, 'web')
            ...
            ->assertRedirect(route('tenant.shipping-zones.index'));
        ...
        $this->actingAs($this->admin, 'web')
            ->post('/shipping-zones/'.$zone->id.'/update', [
        ...
        $this->actingAs($this->admin, 'web')
            ->post('/shipping-zones/'.$zone->id.'/deactivate')
            ->assertRedirect(route('tenant.shipping-zones.index'));
        ...
        $this->actingAs($this->admin, 'web')
            ->post('/shipping-zones/'.$zone->id.'/activate')
            ->assertRedirect(route('tenant.shipping-zones.index'));
    ...
    public function test_admin_can_upsert_and_update_outlet_service_override_from_web_panel(): void
    {
        ...
        $this->actingAs($this->admin, 'web')
            ...
            ->assertRedirect(route('tenant.outlet-services.index'));
        ...
        $this->actingAs($this->admin, 'web')
            ->post('/outlet-services/'.$override->id.'/update', [
                'active' => '0',
    ...
    public function test_admin_cannot_upsert_outlet_service_override_for_out_of_scope_outlet(): void
    {
        ...
        $this->actingAs($this->admin, 'web')
            ...
            ->assertRedirect('/outlet-services')
    ...
    public function test_outlet_service_advanced_filters_by_service_status_and_override_price(): void
    {
        ...
        $this->actingAs($this->admin, 'web')
            ->get('/outlet-services?service_active=0&override_price=none')
            ->assertOk()
    ...
    private function filamentOrderIndexUrl(): string
    {
        return OrderResource::getUrl(name: 'index', panel: 'tenant');
    }

    private function filamentOrderCreateUrl(): string
    {
        return OrderCreate::getUrl(panel: 'tenant');
    }

    private function filamentOrderViewUrl(Order|string $order): string
    {
        return OrderResource::getUrl(name: 'view', parameters: ['record' => $order], panel: 'tenant');
    }
}
